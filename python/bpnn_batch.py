...
class BPNN():
    def __init__(self, dimensions=[784, 100, 10], activation=None):
    ...
    def backward(self, loss_fn, learning_rate=1e-3):
        ...
        for i in reversed(range(len(self.layers))):
            ...
            if i == len(self.layers) - 1:
                ...
                '''
                error = numpy.dot(delta[i + 1], self.layers[i + 1].weights.T)
                delta[i] = error * self.activation[i].backward()
                
        # 更新一波权重
        for i in range(len(self.layers)):
            residual = numpy.mean([l.reshape(-1, 1) * r for l, r in zip(self.layers[i].backward(), delta[i])], axis=0)

            self.layers[i].weights -= learning_rate * residual
            self.layers[i].bias -= learning_rate * numpy.mean(delta[i], axis=0)
...
class Solver(object):
    ...
    def train(self, epoches=1, learning_rate=1e-2, shuffle=True, batch_size=16):
        for epoch in range(epoches):
            with utils.Timer() as time_scope:
                # 训练一个 epoch
                process_bar = tqdm.tqdm(self.loader.mini_batch(batch_size))
                for cnt, (x, y) in enumerate(process_bar):
                    ...
                    process_bar.set_postfix(loss=numpy.mean(numpy.sum(loss_value, axis=-1)))
                ...
                # 保存一次模型
                self.save('./checkpoints/bpnn_epoch_{}_batchsize_{}_accuracy_{}.pkl'.format(epoch + 1, batch_size, score))
...
if __name__ == '__main__':
    ...
    solver.train(epoches=1, learning_rate=3 * 1e-1, batch_size=16)
